"""
Pipeline runner that marries LangGraph execution with database persistence.
...
class PipelineExecutor:
    """Executes the pipeline and records per-agent execution metrics."""

    def __init__(self, db: Session, review_id: str, graph=None):
        self.db = db
        self.review_id = review_id
        self.graph = graph or build_graph()

    def execute(self, state: ReviewState) -> tuple[dict, int, float]:
        final_state: dict = {}
        total_latency_ms = 0
        total_cost_usd = 0.0
        ...
        self.db.commit()
        return final_state, total_latency_ms, total_cost_usd


class ReviewPersistenceService:
    """Persists structured domain objects (Findings, Patches, Verifications) produced by a review."""

    def __init__(self, db: Session, review_id: str):
        self.db = db
        self.review_id = review_id

    def persist_results(self, final_state: dict) -> None:
        ...
        self.db.flush() # Flush to ensure finding IDs are available for PatchSuggestions
...
def run_review_with_observability(
    ...
    *,
    ...
    try:
        executor = PipelineExecutor(db, review_id, graph)
        final_state, total_latency_ms, total_cost_usd = executor.execute(state)

        persistence = ReviewPersistenceService(db, review_id)
        persistence.persist_results(final_state)
